...
thumbnails.forEach(thumbnail => {
    thumbnail.addEventListener('click', event=> {
        ...
        imageContainer.style.backgroundImage = `url('../imagenes/taladro-bosch-${event.target.id.slice(-1)}.png')`;
...
modalThumbnails.forEach(modalThumbnail =>{
    modalThumbnail.addEventListener('click', event=>{
        ...
        modalImageContainer.style.backgroundImage = `url('../imagenes/taladro-bosch-${event.target.id.slice(-1)}.png')`;
    });
});

// Cambiar imagen principal de modal desde las flechas.
...
function drawProductInModal(){
    ...
    <img class="cart-modal__image" src="./imagenes/taladro-bosch-1.png" alt="">
    <div>
    <p class="cart-modal__product">Taladro Bosch Percutor...</p>
    <p class="cart-modal__price">$125 x3 <span>$375.000</span></p>
    </div>
    <img class="cart-modal__delete" src="./imagenes/basura.png" alt="delete">
    </div>
    <button class="cart-modal__chekout">Checkout</button>`
    deleteProduct();
    let priceModal = document.querySelector('.cart-modal__price');
    priceModal.innerHTML = `$125 x ${lastValue} <span>$${lastValue*125}.000</span> `;
...
function changeNextImage(imgContainer){
    ...
    imgContainer.style.backgroundImage = `url('../imagenes/taladro-bosch-${imgIndex}.png')`;
